...
def generate_date_filter(column_parameter: str,
                         date_from: Optional[date] | None,
                         date_to: Optional[date] | None) -> str:
...
def generate_rates_query(origin: Tuple[str, PortColumn] | None,
                         destination: Tuple[str, PortColumn] | None,
                         range_date: Tuple[date | None, date | None]) -> str:
    ...
    if (origin_column, destination_column) == (PortColumn.PARENT_SLUG, PortColumn.PARENT_SLUG):
        query = text(f"""
    ...
    elif origin_column == PortColumn.PARENT_SLUG and \
            destination_column in (PortColumn.CODE, PortColumn.NAME, PortColumn.NONE):

        filter_by_destiny = generate_port_filter(destination_column, 'p2', destination_value)
...
def get_rates(date_from: Optional[date] | None,
              date_to: Optional[date] | None,
              origin: Tuple[str, PortColumn] | None,
              destination: Tuple[str, PortColumn] | None):

    query = generate_rates_query(origin,
                                 destination,
                                 (date_from, date_to))
